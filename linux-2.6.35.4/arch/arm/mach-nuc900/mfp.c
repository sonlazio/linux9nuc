...
void mfp_set_groupg(struct device *dev)
{
	...
	} else if (strcmp(dev_id, "nuc900-i2c-p0") == 0) {
		mfpen &= ~(GPIOG0TO1);
		mfpen |= ENI2C0;/*enable i2c0*/
	} else if (strcmp(dev_id, "nuc900-i2c-p1") == 0) {
		mfpen &= ~(GPIOG2TO3);
		mfpen |= ENI2C1;/*enable i2c1*/
        } else if (strcmp(dev_id, "nuc900-audio-ac97") == 0) {
		mfpen &= ~(GPIOG22TO23);
		mfpen |= ENAC97;/*enable AC97*/
        } else if (strcmp(dev_id, "nuc900-audio-i2s") == 0) {
                mfpen &= ~(GPIOG22TO23);
                mfpen |= ENI2S;/*enable I2S*/
	} else {
