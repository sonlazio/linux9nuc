...
static void nuc900fb_activate_var(struct fb_info *info)
{
	...
	dev_dbg(fbi->dev, "PICOS2KHZ(var->pixclock)=%ld,%ld\n",PICOS2KHZ(var->pixclock),fbi->clk_rate);
	//caculate clock div
	if(PICOS2KHZ(var->pixclock) > fbi->clk_rate/1000)
	{	
		writel(readl(REG_CLKSEL) & ~0xc0 ,REG_CLKSEL);		//video clock source selected from PLL0
		fbi->clk_rate = get_cpu_clk()*1000;						//change clock rate from PLL0				
		dev_dbg(fbi->dev, "select pll0:%x,%ld\n",readl(REG_CLKSEL),fbi->clk_rate);	
		dev_dbg(fbi->dev, "pll0:%x,%x\n",readl(REG_PLLCON0),readl(REG_CLKDIV));	
	}
	
	clkdiv = nuc900fb_calc_pixclk(fbi, var->pixclock) - 1;
	if (clkdiv < 0)
		clkdiv = 0;
	printk("video clkdiv=%d\n", clkdiv);
	writel((readl(REG_CLKDIV) & ~0xf000) | clkdiv << 12, REG_CLKDIV);	
}
